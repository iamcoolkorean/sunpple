...
from auth import authenticate, register_user, get_user_data, save_user_data
...
day_number = weekday + 1  # 1~7

# 요일 범위 초과 처리 (실제로는 월~일만 유효)
if day_number > 7:
...
with st.sidebar:
    ...
    if st.button("로그아웃", use_container_width=True):
        if "messages" in st.session_state:
            save_user_data(st.session_state.user, {"messages": st.session_state.messages})
        st.session_state.authenticated = False
        st.session_state.user = None
        st.session_state.messages = []
        st.rerun()

...
- Day 6 이후에는 반드시 이전 대화에서 수집한 실제 학과, 재능, 관심 산업을 인용하여 답변하세요."""
    ...
    def _truncate_conversation(self, conversation, max_turns=30):  # ← 30으로 증가
...
Phase 1(탐색), Phase 2(경험), Phase 3(진입) 순서로 **실제 해커톤, 공모전, 채용 공고**를 찾아서 구체적인 링크와 함께 안내해드릴게요."""
    ...
    def _get_day_instructions(self, day_number, user_data, messages):
        name = user_data.get("name", "사용자")
        instructions = {
            1: f"""[Day 1] {name}님의 전공, 경험, 잘하는 일, 흥미 수집. 질문 순서대로.""",
            2: f"""[Day 2] 몰입 경험, 유형(보기), 도구, 칭찬.""",
            3: f"""[Day 3] 문제 해결 방식(보기), 의사 결정 방식(보기), 도구.""",
            4: f"""[Day 4] 팀 역할(보기), 갈등 반응(보기), 피드백 반응(보기), 셀프 대화(보기).""",
            5: f"""[Day 5] 관심 산업(보기), 문제 유형(보기), 업무 환경(보기), 마감 반응(보기), 롤모델.""",
            6: f"""[Day 6 - 커리어 가설 (데이터 기반)]
1. 지난 대화를 분석하여 {name}님의 학과, 강점, 관심 산업을 먼저 정리하세요.
2. 3가지 커리어 방향을 (산업 / 역할 / 전망) 목록으로 제시하세요.
3. 사용자가 선택하면, 왜 {name}님에게 맞는지 (실제 학과·재능·성향 인용) 설명하세요.""",
            7: f"""[Day 7 - 로드맵]
선택된 가설로 Phase 1(뉴스레터, 유튜브, 링크드인), Phase 2(실제 해커톤/공모전 링크), Phase 3(채용 플랫폼·기업) 순서로 제공. 마지막에 '당신의 무기' 한 줄."""
        }

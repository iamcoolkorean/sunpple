...
class SunppleCoach:
    def __init__(self):
        # Streamlit Secrets에서 8개 API 키 가져오기
        self.api_keys = []
        for i in range(1, 9):
...
- 예: "지난 4일간 당신은 ___에서 에너지를 얻고, ___에서 에너지가 빠지며, ___ 방식으로 결정을 내리는 모습을 보였어요."
...
- "마케터", "개발자" 같은 일반론이 아니라, "B2B SaaS의 고객 온보딩 스페셜리스트"처럼 구체적으로 제시하세요.
